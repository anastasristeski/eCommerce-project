...
####📊 DataSeeder — how the JSON is processed (detailed)
The DataSeeder is a Spring Boot component that automatically populates the database with products when the application starts. Its main purpose is to take a structured JSON file and map it into the application's relational database with all associated relationships.
...
Data source: Reads from resources/static/data.json.
...
Each attribute can have multiple items representing the possible values.
...
Cart items include values (Map<String,String>) to capture the chosen spec per order item.
